...
public class FileOperationHelper {
    ...
    public boolean canMove(String path) {
        for (FileInfo f : mCurFileNameList) {
            ...
            if (Util.containsPath(f.getFilePath(), path))
                return false;
        }
    ...
    public boolean isFileSelected(String path) {
        synchronized(mCurFileNameList) {
            for (FileInfo f : mCurFileNameList) {
                if (f.getFilePath().equalsIgnoreCase(path))
                    return true;
            }
        }
        return false;
    ...
    public boolean Rename(FileInfo f, String newName) {
        ...
        String newPath = Util.makePath(Util.getPathFromFilepath(f.getFilePath()), newName);
        final boolean needScan = file.isFile();
        try {
            boolean ret = file.renameTo(new File(newPath));
            if (ret) {
                if (needScan) {
                    mOperationListener.onFileChanged(f.getFilePath());
                }
    ...
    private void CopyFile(FileInfo f, String dest) {
        ...
        if (file.isDirectory()) {
            ...
            String destPath = Util.makePath(dest, f.getFileName());
            File destFile = new File(destPath);
            int i = 1;
            while (destFile.exists()) {
                destPath = Util.makePath(dest, f.getFileName() + " " + i++);
            ...
            String destFile = Util.copyFile(f.getFilePath(), dest);
        }
    ...
    private boolean MoveFile(FileInfo f, String dest) {
        ...
        File file = new File(f.getFilePath());
        String newPath = Util.makePath(dest, f.getFileName());
        try {
